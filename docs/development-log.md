...
> 更新日期：2026-07-24
> 当前版本：V0.1.2.1（景点扩充、地图搜索定位与详情介绍折叠已完成）
> 用途：记录当前功能、数据结构、关键决定和历史开发过程，供后续开发快速恢复上下文。
> 阅读提示：第一部分是当前最新基准；第二至第七部分为既有历史阶段记录；第八至第十部分依次记录 V0.1.1、V0.1.2 和 V0.1.2.1。

# 第一部分：当前最新状态（截至 2026-07-24）
...
- 6 个旅行区域，新增未央区。
- 21 个景点，全部具有 GCJ-02 参考坐标。
- 21 个景点均具有统一 `reservation` 数据和来源核实记录。
- V0.1.2 新增的 8 个景点具有 `locationMeta` 坐标维护信息。
...
- 首页地图顶部提供景点搜索，只传入 `places.js` 数据。
- 点击搜索结果后不直接进入详情，而是更新地图中心、缩放级别并显示临时搜索 marker。
- 临时 marker 使用独立 ID，新结果替换旧结果；点击临时 marker 可进入景点详情。
...
- `places.js` 中 21 个景点均具有地址、GCJ-02 坐标、开放时间、门票和统一 `reservation` 对象。
- 原有景点预约核实日期为 `2026-07-22`，V0.1.2 新增景点核实日期为 `2026-07-24`。
- 当前渠道类型分布为：7 个官方微信公众号、2 个官方网页、8 个无需预约、4 个暂未确认。
- V0.1.2 新增景点使用 `locationMeta` 记录坐标系、坐标来源、定位说明和核实日期。
...
- `detail.js` 使用 `isDescriptionExpanded` 保存展开状态，默认值为 `false`。
- `detail.wxml` 始终绑定完整 `place.description`，不截取原始字符串。
- `detail.wxss` 在折叠状态使用四行多行截断，展开后恢复完整内容。
- 用户可点击“查看详细介绍”和“收起介绍”反复切换。
- 预约、地址、地图、导航、推荐理由和注意事项逻辑保持不变。
...
- 已核对 21 个景点全部具有坐标和预约数据；新增 8 个景点具有 `locationMeta`。
- 已核对 9 条美食与 9 个图片目录一一对应；19 张 JPG 均非空、可解码，引用路径和大小写完全一致。
- 已完成公共搜索工具、首页实时搜索、区域内搜索和手风琴状态的 JavaScript 语法与静态状态模拟。
- 已验证首页搜索覆盖全部景点并执行地图定位；区域搜索不会返回其他区域内容，联想结果最多 5 条且名称开头匹配优先。
- detail 介绍折叠的 JavaScript 语法、状态切换、WXML 绑定和四行截断样式已完成静态检查。
...
1. 在微信开发者工具和真机完成 21 个景点信息与预约状态的逐项测试。
2. 根据测试结果决定是否增加预约按钮防重复点击及更严格的状态校验。
3. 设计并实现三日行程页面。
4. 补充具有可靠来源和使用权限的景点图片。
5. 继续核对开放时间、门票、预约和门店信息。
6. 在微信开发者工具与 iPhone 真机回归首页景点定位、区域搜索、键盘和返回流程。
7. 回归 detail 长文本的四行摘要、展开、收起及滚动位置。
...
# 第八部分：V0.1.1 首页地图搜索与定位

## 1. 本阶段范围

- 将首页搜索框移动到地图卡片顶部。
- 首页搜索范围调整为仅包含 `places.js` 景点。
- 点击搜索结果后不进入详情，改为移动地图并显示临时 marker。
- 保持 `search-utils.js` 向后兼容，不影响区域页和独立搜索页。

## 2. 数据流

```text
输入景点名称
→ search-utils.js 生成景点联想
→ 点击结果
→ 根据景点 ID 查询 places.js
...
- 首页精选景点 marker、特殊地点、用户定位和全屏地图保持不变。
- 临时 marker 使用独立 ID，新结果替换旧结果。
- 区域页继续搜索当前区域的景点和美食并跳转详情。

# 第九部分：V0.1.2 景点扩充与信息完善

## 1. 数据扩充

- 区域由 5 个增加到 6 个，新增 `weiyang` 未央区。
- 景点由 13 个增加到 21 个。
- 新增小雁塔（荐福寺塔）、西安博物院、广仁寺、高家大院、大明宫国家遗址公园、青龙寺、兴庆宫公园、汉城湖景区。

## 2. 区域归属

- 雁塔区：青龙寺。
- 碑林区：小雁塔（荐福寺塔）、西安博物院、兴庆宫公园。
- 莲湖区：广仁寺、高家大院。
- 未央区：大明宫国家遗址公园、汉城湖景区。

## 3. 地图坐标

- 新增 8 个景点补充 `address`、`latitude` 和 `longitude`。
- 坐标按 GCJ-02 接入，保持现有地图和搜索逻辑不变。
- 新增 `locationMeta`，记录 `coordinateSystem`、`coordinateSource`、`locationNote` 和 `verifiedAt`。
- `verifiedAt` 保持为 `2026-07-24`。

## 4. 游览与预约信息

- 完善新增景点的 `openTime`、`ticket` 和 `reservation`。
- 不虚构不存在的官方预约入口。
- 对可能变化的信息使用“通常”“以官方公告为准”等提示。
- 清理小雁塔和西安博物院注意事项中的过时“待补充”文本。

## 5. 兼容结果

- 新景点可被公共搜索工具匹配。
- 新景点可进入现有详情页。
- 有效坐标可用于首页搜索定位、景点参考地图和微信地图导航。
- 未修改首页地图逻辑、默认 marker 配置和公共搜索接口。

# 第十部分：V0.1.2.1 detail 介绍折叠

## 1. 优化背景

景点详细介绍内容增加后，默认完整展开会延长页面，用户需要较多滑动才能看到地址、地图、预约和推荐信息。

## 2. 技术实现

- `detail.js` 新增 `isDescriptionExpanded`，默认值为 `false`。
- `toggleDescription()` 在展开与折叠状态间切换。
- WXML 始终绑定原始完整 `place.description`。
- WXSS 使用 `-webkit-line-clamp: 4` 生成约四行摘要。
- 展开后恢复完整文本显示，按钮变为“收起介绍”。

## 3. 兼容结果

- 不修改 `places.js` 中的介绍数据。
- 不影响地址、地图定位、导航、推荐理由、注意事项和预约按钮。
- 相关 JavaScript 语法、状态切换、WXML 绑定和样式结构已完成静态检查。
